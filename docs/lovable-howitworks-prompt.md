# Lovable prompt — clarify "how bravo works" + the Git connection

Paste into the bravo landing project. Keeps the design/voice; rewrites the
...
about what actually happens and how bravo relates to Git. Rewrite it, and add a
short "How it connects to Git" block, as follows.

Add a one-line mental model just under the § 04 heading:
"bravo sits on top of a normal Git repository of Markdown files — that repo is
...
Under the hood: bravo opens a private draft branch and commits each autosave.
You never touch Git.
...
§ 04 — Approve
Required reviewers sign off. bravo squashes all your autosaves into a single,
clean commit on the main branch and deletes the draft — one tidy, fully
attributed entry in the history.

§ 05 — Publish
bravo renders a per-agent context bundle (only the documents each agent is
...
"bravo is a friendly layer over a real Git repo that stays on your infrastructure
...
bravo is the only thing that touches Git, so Context Owners get full version
...
"You (edit · propose · approve) → bravo → Git (branches · commits · main) →
